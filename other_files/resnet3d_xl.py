...
class ResNet(nn.Module):
    ...
    def forward_single(self, x):
        ...
        x = self.conv1(x)
        # print('conv1_x_size():\t', x.size())

        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool1(x)
        # print('maxpool1_x_size:\t', x.size())

        x = self.layer1(x)
        # print('layer1_x_size:\t', x.size())
        x = self.maxpool2(x)
        # print('maxpool2_x_size:\t', x.size())
        x = self.layer2(x)
        # print('layer2_x_size:\t', x.size())
