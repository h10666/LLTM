...
from other_files.nonlocal_helper import Nonlocal
from other_files.resnet_TSM import resnet_TSM
# from other_files.SEAttention import SEAttention
# from other_files.TCR import TCR
# from other_files.text2embed import text2embed
...
class VideoModelGlobalCoordLatentNL(nn.Module):
    ...
    def __init__(self):
        ...
        for i in range(self.nr_nonlocal_layers):
            self.c_nonlocal_fusion.append(nn.Sequential(
                Nonlocal(dim=self.coord_feature_dim,
                         dim_inner=self.coord_feature_dim // 2),
                nn.Conv1d(self.coord_feature_dim, self.coord_feature_dim,
        ...
        self.c_box_feature_fusion = nn.Sequential(
            nn.Linear((self.nr_frames // 2) * self.coord_feature_dim,
                      self.coord_feature_dim, bias=False),
            nn.BatchNorm1d(self.coord_feature_dim),
            nn.ReLU(inplace=True),
            nn.Linear(self.coord_feature_dim,
        ...
        self.classifier = nn.Sequential(
            nn.Linear(self.coord_feature_dim + 2 *
                      self.img_feature_dim, self.coord_feature_dim),
            nn.ReLU(inplace=True),
    ...
    def forward(self, global_img_input, video_label, box_input, box_categories, is_inference=False):
        """
        V: num of videos
        T: num of frames
        P: num of proposals
        ...
        # print('global_img_input:\t', global_img_input.size())
        global_img_input = global_img_input.view(-1, 3, 8, *global_img_input.size()[2:])
        # print('global_img_input:\t', global_img_input.size())
        
        # exit(0)
        bs, _, _, _, _ = global_img_input.shape
        y_i3d, org_features = self.i3D(global_img_input)
        # print('org_features:\t', org_features.size())
        # Reduce dimension video_features - [V x 512 x T / 2 x 14 x 14]
        videos_features = self.conv(org_features)
        # print('videos_features:\t', videos_features.size())
        b = bs
        ...
        box_categories = box_categories.view(
            b * self.nr_boxes * (self.nr_frames // 2))
        box_category_embeddings = self.category_embed_layer(
            box_categories)  # (b*nr_b*nr_f, coord_feature_dim//2)

        bf = self.c_coord_to_feature(box_input)
        # (b*nr_b*nr_f, coord_feature_dim + coord_feature_dim//2)
        bf = torch.cat([bf, box_category_embeddings], dim=1)
        # (b*nr_b*nr_f, coord_feature_dim)
        bf = self.c_coord_category_fusion(bf)

        bf = bf.view(b, self.nr_boxes, self.nr_frames //
                     2, self.coord_feature_dim)

        # spatial message passing (graph)
        # (b, 1, self.nr_frames, coord_feature_dim)
        spatial_message = bf.sum(dim=1, keepdim=True)
        # message passed should substract itself, and normalize to it as a single feature
        # message passed should substract itself
        spatial_message = (spatial_message - bf) / (self.nr_boxes - 1)

        # (b, nr_boxes, nr_frames, 2*coord_feature_dim)
        bf_message_gf = torch.cat([bf, spatial_message], dim=3)

        # (b*nr_boxes*nr_frames, coord_feature_dim)
        bf_spatial = self.c_spatial_node_fusion(bf_message_gf.view(
            b * self.nr_boxes * (self.nr_frames // 2), -1))
        bf_spatial = bf_spatial.view(
            b, self.nr_boxes, self.nr_frames // 2, self.coord_feature_dim)
        ...
        bf_nonlocal = bf_nonlocal.view(b, self.nr_boxes, self.coord_feature_dim).permute(
            0, 2, 1).contiguous()  # (N, C, NB)
        for i in range(self.nr_nonlocal_layers):
        ...
        _gf = videos_features.mean(-1).mean(-1).view(b,
                                                     (self.nr_frames//2), 2*self.img_feature_dim)
        _gf = _gf.mean(1)
        video_features = torch.cat([_gf.view(b, -1), coord_ft], dim=-1)

        cls_output = self.classifier(video_features)  # (b, num_classes)
        print('cls_output:\t', cls_output.size())
        return cls_output
...
class VideoRegionAndCoordModel_TSM(nn.Module):
    ...
    def __init__(self, partial_bn=True, fc_lr5=False,
        ...
        self.fc_lr5 = not (args.tune_from and args.dataset in args.tune_from)
        # self.i3D = Net(self.nr_actions, extract_features=True, loss_type='softmax')
        self.backbone = resnet_TSM(args.basic_arch, args.shift, num_segments=8, download_root = args.download_root)
        # self.fusion_mode = 'late_fusion'  # 'early_fusion'
        
        self.avgpool = nn.AdaptiveAvgPool3d((1, 1, 1))
        # self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.conv = nn.Conv3d(2048, self.img_feature_dim*2, kernel_size=(1, 1, 1), stride=1)
        # self.conv = nn.Conv3d(1024, self.img_feature_dim, kernel_size=(1, 1, 1), stride=1)
    ...
    # def forward(self, global_img_input, local_img_input, box_input, video_label, is_inference=False):
    def forward(self, input, box_input, box_categories):
        ...
        global_features = conv_fea_maps.mean(-1).mean(-1).view(V, self.nr_frames, self.img_feature_dim*2)
        ...
        output = torch.cat([coord_feas, global_features], dim = 1)
...
class VideoRegionAndCoordModel_TSM4(nn.Module):
    ...
    def __init__(self, partial_bn=True, fc_lr5=False,
        ...
        self.region_vis_embed = nn.Sequential(
            # nn.Linear(self.img_feature_dim * self.crop_size1[0] * self.crop_size1[1], hidden_feature_dim),
            nn.Linear(512 * self.crop_size1[0] * self.crop_size1[1], hidden_feature_dim),
            nn.ReLU(inplace=True),
            nn.Dropout(0.5)
        )
        ############################################# coord ###############################################
        ...
        self.global_vis_feas_fusion = nn.Sequential(
            nn.Linear(hidden_feature_dim * 8, hidden_feature_dim * 4, bias=False),
            nn.BatchNorm1d(hidden_feature_dim * 4),
            nn.ReLU(inplace=True),
            nn.Linear(hidden_feature_dim * 4, 2048, bias=False),
            nn.BatchNorm1d(2048),
            nn.ReLU(inplace=True)
            )
        ###################################################################################################
        ...
        self.node_fusion = nn.Sequential(
            nn.Linear(hidden_feature_dim * 4, hidden_feature_dim * 2, bias=False),
            nn.BatchNorm1d(hidden_feature_dim * 2),
            nn.ReLU(inplace=True),
            nn.Linear(hidden_feature_dim * 2, hidden_feature_dim, bias=False),
            nn.BatchNorm1d(hidden_feature_dim),
            nn.ReLU(inplace=True)
            )
        # self.classifier = nn.Sequential(
    ...
    def forward(self, input, box_input, box_categories):
        ...
        # mix_feature = torch.cat([global_features, output], dim = -1)
        output = self.mix_feature_fusion(output)
        # ################################
        # print('output_size:\t', output.size())
        # exit(0)
