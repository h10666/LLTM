...
from CLIP.clip.model import CLIP, convert_weights
...
from archs.TRNmodule import RelationModuleMultiScale

args, _ = parser.parse_known_args()
device = "cuda" if torch.cuda.is_available() else "cpu"

###两种方案：1）用CLIP处理video；2）用TSM处理video

class Video_CLIP(CLIP):
    def __init__(self, mode,
        ...
        self.mode = mode ## clip_based
        self.avgpool = nn.AdaptiveAvgPool1d(1)
        self.visual_head_type = visual_head_type ## pool
        self.input_size = 224
    ...
    def forward(self, video, text_input, class_names_input):
        ...
        print('text:\t', text_input.size(),type(text_input))
